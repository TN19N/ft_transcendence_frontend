    ...
    <div className='flex flex-col w-full m-auto text-white iphone:text-[13px] tablet:text-[18px] laptop:text-[20px] imac:text-[24px] gap-3'>
        {blockedUsers.length !== 0 ? (
          blockedUsers.map((blockedUser) => (
            <div className='flex  iphone:[90%] tablet:w-[70%] m-auto gap-3 bg-background p-2 rounded-xl justify-evenly'>
            <div key={blockedUser.id} className='flex items-center gap-2'>
              <ButtonAvatar id={blockedUser.id} />
              <span>{blockedUser.name}</span>
              </div>
                <button className='bg-buttonPlaybgColor rounded-[50%]' onClick={() => unblockUser(blockedUser.id)}>
          ...
          <span>Blocked list is empty</span></div>
        )}
    </div>
  );
}
